...
def analyze_resolution(filename, voltage):
    ...
    header = "V\tΔCN1\tCN1\tr1(%)\n"
    line = f"{voltage}\t{deltaCN1:.4f}\t{mu1:.4f}\t{r1:.4f}\n"
    ...
    plt.scatter(x1, y1, color='blue', label='Data', s=15)
    plt.plot(x1, gauss_linear(x1, *popt1), 'r-', lw=2, label='Fit')
    plt.title(f"Peak Fit ({voltage} V)")
...
def process_all_files(folder='.'):
    files = sorted([f for f in os.listdir(folder) if f.startswith("Co") and f.endswith("Peak.txt")])
    if not files:
